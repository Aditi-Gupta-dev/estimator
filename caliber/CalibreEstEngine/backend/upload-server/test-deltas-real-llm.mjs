...
async function waitForTerminal(cookie, estimateId, maxTries = 25, delayMs = 2000) {
  let delta = null;
...
async function main() {
  ...
  const createRes = await post(estimator.cookie, '/api/estimates', {
    name: 'Phase 5.1 Real E2E', businessUnit: 'Oracle ERP',
    inputs: sampleInputs(['Enterprise Structures & Legal Entities', 'Integration - Simple']),
  });
  ...
  const saveRes = await patch(estimator.cookie, `/api/estimates/${e1.id}`, {
    persist: true,
    changes: { moduleEffortMultiplier: { Integration: 1.3 }, onshorePctDelta: 20, durationMonthsDelta: 0 },
  });
  check('V2 created from V1 (persist:true)', saveRes.status === 200 && saveRes.data.result?.latestVersion?.version === 2);
  const v2 = saveRes.data.result.latestVersion;
  console.log(`    V1->V2 previousVersionId=${v2.previousVersionId}`);

  console.log('  waiting for REAL delta analysis (PENDING -> RUNNING -> COMPLETED/FAILED)...');
  const delta = await waitForTerminal(estimator.cookie, e1.id);

  check('Delta reached a terminal state', delta && (delta.status === 'completed' || delta.status === 'failed'));
  check('Deterministic delta computed before AI ran', Array.isArray(delta?.deterministicDelta?.numericDeltas) && delta.deterministicDelta.numericDeltas.length > 0);
  ...
  if (delta.status !== 'completed') {
    ...
    check('AI analysis has all required fields', ['summary', 'key_changes', 'likely_drivers', 'impact', 'risks', 'recommendations', 'confidence', 'scope_creep_indicated'].every((k) => k in ai));
    check('confidence is a valid enum value', ['low', 'medium', 'high'].includes(ai.confidence));
    ...
    delta.deterministicDelta.numericDeltas.forEach((d) => { detNumbers.add(String(d.previous)); detNumbers.add(String(d.current)); });
    const keyChangesText = ai.key_changes.join(' ');
    const anyRealNumberReferenced = [...detNumbers].some((n) => keyChangesText.includes(n));
    check('key_changes reference real deterministic numbers (not obviously invented)', anyRealNumberReferenced, `det numbers: ${[...detNumbers].join(',')} | text: ${keyChangesText}`);
    check('model does not claim it performed the calculation', !/\bi calculated\b|\bi computed\b/i.test(JSON.stringify(ai)));

    // ── Part 7: rate-card security with REAL LLM ─────────────────────────
    console.log('\nPart 7: rate-card security (real LLM run)');
    const estimatorView = await get(estimator.cookie, `/api/deltas/${delta.id}`);
    const adminView = await get(admin.cookie, `/api/deltas/${delta.id}`);
    const estimatorHasRoleRate = estimatorView.data.delta.deterministicDelta.numericDeltas.some((d) => d.category === 'roleRate');
    const adminHasRoleRate = adminView.data.delta.deterministicDelta.numericDeltas.some((d) => d.category === 'roleRate');
    check('estimator-facing API response has NO roleRate entries', !estimatorHasRoleRate);
    console.log(`    (admin roleRate present: ${adminHasRoleRate} — confirms whether real rate data existed to redact)`);
    // The AI's own text must never surface a specific day-rate figure (a $ amount per day).
    const aiText = JSON.stringify(ai);
    const looksLikeRateFigure = /\$\s?\d{2,4}\s*(\/|per)\s*day/i.test(aiText) || /blended\s*rate/i.test(aiText);
    check('AI analysis text contains no rate-card figures or the term "blended rate"', !looksLikeRateFigure, aiText);
  }
  ...
  const listBefore = await get(estimator.cookie, `/api/estimates/${e1.id}/deltas`);
  check('exactly one delta row exists for the V1->V2 pair', listBefore.data.deltas.length === 1);

  // ── Part 10: audit ────────────────────────────────────────────────────
  console.log('\nPart 10: audit trail');
  const audit = await get(estimator.cookie, `/api/estimates/${e1.id}/audit`);
  const actions = audit.data.events.map((e) => e.action);
  check('DELTA_ANALYSIS_REQUESTED recorded', actions.includes('DELTA_ANALYSIS_REQUESTED'));
  check(`DELTA_ANALYSIS_${delta.status === 'completed' ? 'COMPLETED' : 'FAILED'} recorded`, actions.includes(`DELTA_ANALYSIS_${delta.status === 'completed' ? 'COMPLETED' : 'FAILED'}`));
  const deltaEvents = audit.data.events.filter((e) => e.action.startsWith('DELTA_ANALYSIS'));
  check('audit metadata contains no raw delta contents or secrets', deltaEvents.every((e) => {
    const s = JSON.stringify(e.metadata || {});
    return !s.includes('numericDeltas') && !s.toLowerCase().includes('api_key') && !s.toLowerCase().includes('gsk_');
  }));

  // ── Part 11: notifications ────────────────────────────────────────────
  console.log('\nPart 11: notifications');
  const notifs = await get(estimator.cookie, '/api/notifications');
  const expectedType = delta.status === 'completed' ? 'delta_analysis_completed' : 'delta_analysis_failed';
  check(`notification of type ${expectedType} created for estimate owner`, notifs.data.notifications.some((n) => n.type === expectedType && n.estimateId === e1.id));
